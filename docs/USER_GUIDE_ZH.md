...
这份文档给第一次接触本插件的人使用。目标是用最短路径在 Wwise 里听到 v0.2 的风声、雨声、材质响应，并用 2D Preview 验证 Listener 与圆形 Feature 的交互。

当前版本是 `v0.2 executable slice`，固定支持：
...
它还不包含游戏侧 Runtime 接口、Unity/Unreal Adapter、完整 Deflector/Aperture 风场、雷暴、Listener Path、Capture/Replay、Monitor 或 Ambisonics。当前听感目标是“物理启发、可调、可回归”，不是实时全物理仿真；自动生成的 WAV、频谱或 smoke 指标也不等同于真人主观听感批准。
...
| 目的 | 从哪里开始 |
| --- | --- |
| 只想尽快听到效果 | 安装 DLL/XML，然后打开仓库内的 `WwiseSmoke` 工程 |
...
安装脚本会校验 staging hash；如果目标位置已有同名文件，会先备份到 `Artifacts\InstallBackup`。
...
| --- | --- |
| X/Y/Z | Feature 中心位置 |
| Radius | 圆形/SphereProxy 半径，必须大于 0 |
| Profile | 材质响应编号，见下表 |
| Mask | 是否参与雨/风模块 |
...
不同 Wwise 布局或语言下菜单位置可能不同，但需要创建的是 Source Plug-in，而不是 Effect Plug-in 或 Audio Device。
...
自动 smoke 会复制仓库内 fixture 工程到 `Build\WwiseSmoke\Projects\<timestamp>` 的 disposable 目录，启动这份副本，创建插件 Source，执行 GUI smoke、播放、读取 Voice/CPU/Output Peak，并保存 `.prof`。运行前必须关闭其他 Wwise 实例：
...
- `inspectorTextMatchesStablePreset = true`，13 个可见 Inspector 文本匹配 stable preset：`60/0.75/24681357/20/4 / 8/14/35/0.65/Feature1 0,6,2.5,3,10`
- Add 完整默认值断言为 true
- Geometry `true -> false` 后 Undo 恢复
- Add、Feature 圆心拖动、半径拖动、Delete button、Delete key 均有 Undo 还原证据；最终 smoke 合计覆盖 Add/drag/radius/Delete 和 6 个 Undo gate
- Wwise 实际 SoundBank 生成与 Authoring 参数序列化通过：true/false bank 各 `457` bytes，`273`-byte 参数块在 bank offset `77` 唯一匹配，Geometry 为 block offset `16` / bank offset `93`，true=`1`、false=`0`
- `transportState = playing`
- 存在启动且非虚拟的 Voice；最终证据为 1 个 physical voice
- `OutputPeak` 高于 silence floor；最终证据为 `-28.6425437927246 dB`
- 插件 Source CPU 有 Profiler 行；最终证据为 `0.1396999955177307 ms`
- `profilerCaptureSaved = true`；最终 `.prof` 为 `368867` bytes
...
当前 v0.2 已验证 Wwise 实际 SoundBank 生成与 Authoring 参数序列化，但尚未验证生成 bank 由独立 Native SoundEngine Host 加载/执行。现阶段已经验证的是 Wwise Authoring Source、GUI smoke、SoundBank 生成/序列化、Transport、Profiler 与离线 DSP；游戏 C ABI、Scene Snapshot、Native SoundEngine Host 运行属于下一里程碑。
